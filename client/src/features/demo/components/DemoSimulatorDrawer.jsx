...
export function DemoSimulatorDrawer({ isOpen, onClose, onRefreshData }) {
  ...
  const runScenario = async (scenarioId) => {
    if (!VALID_SCENARIOS.has(scenarioId)) {
      setError('Invalid demo scenario identifier.');
      return;
    }
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-white border-l border-slate-200 shadow-2xl flex flex-col animate-slideInRight">
      {/* Header */}
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-slate-50/70">
        <div className="flex items-center space-x-2">
          <Play className="w-4 h-4 text-indigo-600 fill-indigo-600" />
          <h2 className="text-sm font-bold text-slate-900 tracking-wide uppercase">5-Minute Pitch Simulator</h2>
        </div>
        <button 
          onClick={onClose} 
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 text-xs">
        <div className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-lg">
          <div>
            <div className="font-semibold text-slate-900">Reset Demo Baseline</div>
            <div className="text-slate-500">Clears database and resets to clean initial state.</div>
          </div>
          <button
            onClick={() => runScenario('reset')}
            disabled={running !== null}
            className="flex items-center space-x-1 px-3 py-1.5 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 rounded font-medium disabled:opacity-50 transition shadow-2xs"
          >
        ...
        <div className="text-slate-500 font-semibold tracking-wider uppercase text-[10px] pt-2 font-mono">
        ...
        <div className="space-y-3">
          {scenarios.map((sc) => {
            ...
            return (
              <div
                key={sc.id}
                className="p-3.5 bg-white border border-slate-200 hover:border-indigo-300 rounded-lg transition shadow-2xs group"
              >
                <div className="flex items-start justify-between mb-1.5">
                  <div className="flex items-center space-x-2">
                    <span className="font-mono text-[10px] font-bold px-1.5 py-0.5 rounded bg-slate-100 text-indigo-700 border border-slate-200">
                      {sc.badge}
                    </span>
                ...
                <button
                  ...
                  disabled={running !== null}
                  className="w-full flex items-center justify-center space-x-1.5 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 rounded font-medium text-xs transition disabled:opacity-50"
                >
                  {isCurrent ? (
        ...
        {/* Output Console */}
        {(lastResult || error) && (
          <div className="mt-4 p-3 bg-slate-900 border border-slate-800 rounded-lg font-mono text-[11px] text-slate-200">
